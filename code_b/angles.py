...
def angle_hip(hip_l, hip_r):
    hip_v = np.array([hip_l.x - hip_r.x, hip_l.y - hip_r.y, hip_l.z - hip_r.z], dtype=np.float16)
...
def angle_ground(left, right):
    vector = np.array([left.x - right.x, left.y - right.y, left.z - right.z], dtype=np.float16)
...
def back_angle(shoulder_l, shoulder_r):
    sl = np.array([shoulder_l.x, shoulder_l.y, shoulder_l.z], dtype=np.float16)
    sr = np.array([shoulder_r.x, shoulder_r.y, shoulder_r.z], dtype=np.float16)
    connection = sl - sr
...
def mass_balance(foot_l, foot_r):
    left = np.array([foot_l.x, foot_l.y, foot_l.z], dtype=np.float16)
    right = np.array([foot_r.x, foot_r.y, foot_r.z], dtype=np.float16)
...
def pelvis_rotation(hip_l, hip_r, r):
    hip_v = r @ np.array([hip_l.x - hip_r.x, 0, hip_l.z - hip_r.z], dtype=np.float16)
    normal = np.array([0, 0, 1])
    # angle = np.arccos(normal.dot(hip_v) / (np.linalg.norm(normal) * np.linalg.norm(hip_v)))
...
def pelvis_tilt(hip_l, hip_r, r):
    hip_v = r @ np.array([hip_l.x - hip_r.x, hip_l.y - hip_r.y, hip_l.z - hip_r.z], dtype=np.float16)
    normal = np.array([0, 1, 0])
    angle = np.arccos(normal.dot(hip_v) / (np.linalg.norm(normal) * np.linalg.norm(hip_v)))
...
def pelvis_sway(foot_l, r):
    hip_v = r @ np.array([0, 0, foot_l.z], dtype=np.float16)

    return hip_v[2]


def pelvis_thrust(foot_l, r):
    hip_v = r @ np.array([foot_l.x, 0, 0], dtype=np.float16)

    return hip_v[0]


def pelvis_lift(foot_l, r):
    hip_v = r @ np.array([0, foot_l.y, 0], dtype=np.float16)

    return hip_v[1]


def thorax_rotation(shoulder_l, shoulder_r, r):
    shoulder_v = r @ np.array([shoulder_l.x - shoulder_r.x, 0, shoulder_l.z - shoulder_r.z], dtype=np.float16)
    normal = np.array([0, 0, 1])
...
def thorax_tilt(shoulder_l, shoulder_r, r):
    shoulder_v = r @ np.array([shoulder_l.x - shoulder_r.x, shoulder_l.y - shoulder_r.y, shoulder_l.z - shoulder_r.z], dtype=np.float16)
    shoulder_l = r @ np.array([shoulder_l.x, shoulder_l.y, shoulder_l.z], dtype=np.float16)
...
def thorax_sway(shoulder_l, shoulder_r, foot_l, r):
    shoulder_v = r @ np.array([0, 0, shoulder_l.z - shoulder_r.z], dtype=np.float16)
    shoulder_l = r @ np.array([0, 0, shoulder_l.z], dtype=np.float16)
    foot_l = r @ np.array([0, 0, foot_l.z], dtype=np.float16)
    spine = shoulder_l - 0.5 * shoulder_v
    sway = spine - foot_l
...
def thorax_thrust(shoulder_l, shoulder_r, foot_l, r):
    shoulder_v = r @ np.array([shoulder_l.x - shoulder_r.x, 0, 0], dtype=np.float16)
    shoulder_l = r @ np.array([shoulder_l.x, 0, 0], dtype=np.float16)
    foot_l = r @ np.array([foot_l.x, 0, 0], dtype=np.float16)
    spine = shoulder_l - 0.5 * shoulder_v
    thrust = spine - foot_l
...
def thorax_lift(shoulder_l, shoulder_r, foot_l, r):
    shoulder_v = r @ np.array([0, shoulder_l.y - shoulder_r.y, 0], dtype=np.float16)
    shoulder_l = r @ np.array([0, shoulder_l.y, 0], dtype=np.float16)
    foot_l = r @ np.array([0, foot_l.y, 0], dtype=np.float16)
    spine = shoulder_l - 0.5 * shoulder_v
    lift = spine - foot_l
...
def head_rotation(eye_l, eye_r, r):
    eye_v = r @ np.array([eye_l.x - eye_r.x, 0, eye_l.z - eye_r.z], dtype=np.float16)
    normal = np.array([0, 0, 1])
...
def head_tilt(eye_l, eye_r, r):
    eye_v = r @ np.array([eye_l.x - eye_r.x, eye_l.y - eye_r.y, eye_l.z - eye_r.z], dtype=np.float16)
    eye_v[0] = 0
    normal = np.array([0, 0, 1])
    angle = np.arccos(normal.dot(eye_v) / (np.linalg.norm(normal) * np.linalg.norm(eye_v)))
...
def left_arm_length(shoulder_l, shoulder_r, wrist_l, r):
    shoulder_v = r @ np.array([shoulder_l.x - shoulder_r.x, shoulder_l.y - shoulder_r.y, shoulder_l.z - shoulder_r.z], dtype=np.float16)
    shoulder_l = r @ np.array([shoulder_l.x, shoulder_l.y, shoulder_l.z], dtype=np.float16)
    wrist_l = r @ np.array([wrist_l.x, wrist_l.y, wrist_l.z], dtype=np.float16)
    spine = shoulder_l - 0.5 * shoulder_v
...
def wrist_angle(pinky_l, index_l, wrist_l, elbow_l, r):
    pinky_l = r @ np.array([pinky_l.x, pinky_l.y, pinky_l.z], dtype=np.float16)
    index_l = r @ np.array([index_l.x, index_l.y, index_l.z], dtype=np.float16)
    wrist_l = r @ np.array([wrist_l.x, wrist_l.y, wrist_l.z], dtype=np.float16)
    elbow_l = r @ np.array([elbow_l.x, elbow_l.y, elbow_l.z], dtype=np.float16)
